...
=====================
Implementation of the BaseExchange class for the MEXC exchange.
Handles MEXC-specific functionality and WebSocket connections.
"""
...
class MexcExchange(BaseExchange):
    ...
    def __init__(
        ...
        """
        Initialize the MEXC exchange connector.
        
        Args:
            api_key: API key for MEXC
            api_secret: API secret for MEXC
    ...
    def has_websocket_support(self) -> bool:
        """
        Check if MEXC has WebSocket support.
        
        Returns:
            bool: True as MEXC supports WebSockets
        """
        return True
    ...
    async def create_order(
        ...
        # Convert order types to MEXC-specific format
    ...
    async def set_leverage(self, symbol: str, leverage: int) -> Dict[str, Any]:
        ...
        try:
            # For MEXC, the API endpoint might be different
            market_id = self.exchange.market_id(symbol)
            response = await self.exchange_async.contractPrivatePostPositionChangePositionLeverage({
                'symbol': market_id,
                'leverage': leverage
            })
    ...
    async def set_margin_type(self, symbol: str, margin_type: str) -> Dict[str, Any]:
        ...
        try:
            market_id = self.exchange.market_id(symbol)
            # Note: MEXC API endpoint may differ from Binance
            response = await self.exchange_async.contractPrivatePostPositionSwitchPositionMode({
                'symbol': market_id,
    ...
    async def get_funding_rate(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            market_id = self.exchange.market_id(symbol)
            response = await self.exchange_async.contractPublicGetMarketFundingRate({
                'symbol': market_id
            })
            return response
        except Exception as e:
